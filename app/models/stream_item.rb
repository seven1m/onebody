class StreamItem < ActiveRecord::Base
  ...
  def self.shared_with(person)
    ...
    .where("person_id != :id", id: person.id)
    .where(
      "(group_id in (:group_ids) or (group_id is null and person_id in (:friend_ids)) or streamable_type in ('NewsItem', 'Site', 'Person') or is_public = :true)",
