class AttendanceRecord < ActiveRecord::Base
  ...
  def self.daily_counts(limit, offset, date_strftime='%Y-%m-%d', only_show_date_for=nil)
    [].tap do |data|
      counts = connection.select_all("select count(date(attended_at)) as count, date(attended_at) as date from attendance_records where site_id=#{Site.current.id} group by date(attended_at) order by attended_at desc limit #{limit} offset #{offset};").group_by { |p| Date.parse(p['date'].strftime('%Y-%m-%d')) }
