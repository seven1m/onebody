...
class CommentsControllerTest < ActionController::TestCase
  ...
  should "add a comment to a verse" do
    ...
    post :create, {:text => 'dude', :verse_id => @verse.id}, {:logged_in_id => @person}
  ...
  should "add a comment to a recipe" do
    @recipe = Recipe.forge
    num_comments = Comment.count
    post :create, {:text => 'dude', :recipe_id => @recipe.id}, {:logged_in_id => @person}
    assert_response :redirect
    assert_equal num_comments + 1, Comment.count
  end
  ...
  should "add a comment to a note" do
    ...
    num_comments = Comment.count
    post :create, {:text => 'dude', :note_id => @note.id}, {:logged_in_id => @person}
    assert_response :redirect
    assert_equal num_comments + 1, Comment.count
  end
