...
describe MessagesController, type: :controller do
  ...
  it 'should create new private messages' do
    ...
    post :create,
         { message: { to_person_id: @person.id, subject: 'Hello There', body: 'body' } }, logged_in_id: @other_person.id
    expect(response).to be_success
  ...
  it 'should render preview of private message' do
    ActionMailer::Base.deliveries = []
    post :create,
         { format: 'js', preview: true, message: { to_person_id: @person.id, subject: 'Hello There', body: 'body' } },
  ...
  it 'should render preview of group message' do
    ActionMailer::Base.deliveries = []
    post :create,
         { format: 'js', preview: true, message: { group_id: @group.id, subject: 'Hello There', body: 'body' } },
  ...
  it 'should create new group messages with an attachment' do
    ...
    expect(response).to be_success
    post :create,
         { files: [Rack::Test::UploadedFile.new(Rails.root.join('spec/fixtures/files/attachment.pdf'),
  ...
  it 'should not allow parent_id on message user cannot see' do
    ...
    expect(response).to be_forbidden
    post :create,
         { message: { to_person_id: @other_person.id, subject: 'Hello There', body: 'body', parent_id: @message.id } },
         logged_in_id: @person.id
    expect(response).to be_forbidden
  end
  ...
  it 'should send message to members of group that are selected' do
    post :create,
         { message: { member_ids: [@peers[0].id, @peers[2].id], subject: 'is peer1 OK?',
                      body: 'peer1 needs prayer', group_id: @group.id } },
         logged_in_id: @person.id
    expect(response).to redirect_to(@group)
    message = Message.last
    expect(@peers[1].can_read? message).not_to be
  end
end
