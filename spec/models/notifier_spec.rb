...
describe Notifier do
  ...
  context 'given a private message between two people' do
    ...
    it 'should deliver from user address' do
      expect(@sent.from.first).to eq(@user.email)
    end
    ...
    context 'given a reply message' do
      ...
      it 'should deliver from user email address' do
        expect(@sent.from.first).to eq(@user2.email)
      end
    end
