...
Modalbox.Methods = {
	...
	options: {
		...
		align: 'center', // Default alignment, also accepts 'left' or 'right'
		margin: 50, // Default pixels between window and edge of screen (if using 'left' or 'right' alignment)
	...
	hide: function(options) { // External hide method to use from external HTML and JS
		if(this.initialized) {
			...
			if(this.options.transitions) {
				...
				new Effect.Move(this.MBwindow, { mode: 'absolute', x: parseInt($(this.MBwindow).style.left), y: ($(this.MBwindow).getHeight()-30)*-1, duration: this.options.slideUpDuration, afterFinish: this._deinit.bind(this) } );
			} else {
				$(this.MBwindow).hide();
				this._deinit();
			}
	...
	resize: function(byWidth, byHeight, options) { // Change size of MB without loading content
		...
		if(this.options.transitions) {
		  if(this.options.align == 'right') {
		    var left = oWidth - newWidth - this.options.margin;
		  } else if(this.options.align == 'left') {
		    var left = this.options.margin;
		  } else {
		    var left = (oWidth - newWidth)/2;
		  }
			new Effect.Morph(this.MBwindow, {
				style: "width:" + newWidth + "px; height:" + newHeight + "px; left:" + left + "px",
	...
	{	
		...
		if(this.options.transitions) {
			Effect.toggle(this.MBoverlay, 'appear', {duration: this.options.overlayDuration, afterFinishy: this._removeElements.bind(this) });
		} else {
			this.MBoverlay.hide();
	...
	_setPosition: function () {
	  ...
	  var wWidth = $(this.MBwindow).getWidth()
	  if(this.options.align == 'right') {
	    var left = oWidth - wWidth - this.options.margin;
	  } else if(this.options.align == 'left') {
	    var left = this.options.margin;
	  } else {
	    var left = (oWidth - wWidth)/2;
	  }
		$(this.MBwindow).setStyle({left: left + "px"});
	},
